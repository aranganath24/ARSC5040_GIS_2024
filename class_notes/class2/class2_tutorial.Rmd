...
Please load the following libraries. Remember to first install those packages you have not yet installed using the ```install.packages()``` function. 
...
In addition, please turn off spherical geometries within the ```sf()``` package using the syntax we learned in the previous lesson: 
...
We will use a variety of datasets, from a variety of different sources, in this lesson. These datasets and sources will be introduced over the course of the lesson. 

# Layering Data 

Recall, from last class, that one of the most powerful features of GIS software is the ability to layer different spatial datasets on top of each other, which can help us to visualize spatial relationships. To see how this works, let's layer two vector layers on top of each other The first layer is a polygon dataset of the world's country boundaries, which you will recall from last class. Let's extract this dataset and visualize it below:
...
Now, let's take another vector dataset, and layer it over these polygons. In particular, we'll work with a global dataset of World Bank foreign aid projects, available through the AidData Project, available at [https://www.aiddata.org/data/world-bank-geocoded-research-release-level-1-v1-4-2](https://www.aiddata.org/data/world-bank-geocoded-research-release-level-1-v1-4-2). 
...
```{r, message=F}
# Reads in processed dataset of World Bank project sites from cloud storage
...
Go ahead and open up the dataset by passing the name of the object to the data viewer, and have a look:
...
You'll see that there is a "latitude" field and a "longitude" field, which together define each World Bank project's location on the Earth's surface, based on the WGS1984 geographic coordinate system (i.e. the coordinate reference system used by GPS devices, and often the default coordinate reference system for GIS datasets). However, the dataset itself is not yet a spatial object; it's actually a regular data frame, which we can confirm with the following:   
...
We can confirm that ```worldbank_locations_spatial``` is indeed a spatial ("sf") object with the following:
...
```{r, eval=F}
# Explicitly sets "country_boundaries" to the WGS84 coordinate system
...
```{r}
# Sets the coordinate reference system of "country_boundaries" to that of "worldbank_locations_spatial"
...
```{r, fig.asp=0.5}
# plots world bank project sites
tm_shape(worldbank_locations_spatial)+
  tm_dots()
```
...
Note that order matters. We can think of each spatial representation as its own layer, with layers added on top of each other. So, in the code above, the ```country_boundaries``` layer is the first (bottom-most) layer, and the ```worldbank_locations_spatial``` is the second layer that is superimposed on top of the first. If we were to reverse the order, our country polygons would obscure most of our points (assuming that the polygons are not made transparent): 
...
To confirm that the Malawi constituencies polygon is in the same coordinate reference system as the point layer of World Bank project sites, we can print the dataset's metadata:

```{r}
# prints "Malawi_constituencies" metadata
Malawi_constituencies
```
...
The direction of the spatial join is largely determined by the context of the problem, or question, that is under investigation; there is no "right" direction (here, spatially joining ```malawi_foreignaid_sites_projected``` to ```Malawi_constituencies_projected``` is just as valid as the inverse operation of spatially joining  ```Malawi_constituencies_projected``` to ```malawi_foreignaid_sites_projected```; the appropriate option depends on the question one is trying to answer). 
...
```{r}
# Takes "malawi_constituencies_projects_spatial", replaces "na" values for "sum_projects" column with zero values, and relocates ID and "sum_projects" to the front of the dataset using the "relocate" function
...
In addition to using GIS tools to analyze vector datasets in terms of their spatial relations, we can also manipulate or transform vector datasets for purposes of visualization or analysis. This section considers some of these common vector data operations. Such operations are often called "geoprocessing". 
...
# extracts a polygon layer of tract level data for Colorado and assigns to object named "CO_census_tracts"
...
```{r}
# Defines bounding box and assigns it to a new object named "co_mex_bbox"
...
Let's first extract a state-level spatial dataset of population (based on the 2010 decennial census) using *tidycensus*, project it into a projected coordinate reference system ([https://epsg.io/5070-1252](https://epsg.io/5070-1252), and assign it to an object named ```usa_state_boundaries```:
...
To make things somewhat easier, let's focus on the states in the continental United States. To do so, we'll remove Alaska, Hawaii, and Puerto Rico from the dataset using their census-defined GEOID codes. We'll assign the resulting data to a new object named ```continental_USA```:
...
Now that we've added a categorical region variable to ```continental_USA_regions``` (which you can confirm by opening up the dataset with the ```View()``` function), let's now collapse the ```continental_USA_regions``` by region, using the ```group_by()``` and ```summarise()``` functions; we'll assign this collapsed (i.e. "dissolved") dataset to a new object named ```continental_usa_dissolve```:

```{r}
# Dissolves state boundaries by regional groupings and assigns the result to a new object named "continental_usa_dissolve"
...
Note the difference from the earlier plot; instead of using the bounding box as a cookie cutter, it returns the full extent of all the polygons that intersect with the bounding box. 
...
```{r}
# plots "CO_epa_boulder_points" against the backdrop of Boulder county polygon
...
You'll notice that some of the locations fall outside of the area delimited by the Boulder county polygon; this could be due to error of various kinds. Instead of diagnosing it, let's just say we want to clip out the points outside of Boulder. 
...
```{r}
# maps cleaned dataset of Boulder county EPA sites against Boulder County polygon
...
# assigns vector dataset of hospitals to new spatial object named "hospital_points"
...
```{r}
# Plots hospital locations against backdrop of Colorado counties
tm_shape(CO_counties)+
  tm_polygons()+
...
```{r}
# Extracts hospitals in Boulder and assigns these observations to a new object named "boulder_hospitals"
...
```{r}
# Plots Boulder hospitals against the backdrop of Boulder county
tm_shape(boulder_county_projected)+
  tm_polygons()+
...
Now, let's say we want to calculate the Euclidean distance between all of the hospitals in Boulder. We can use the ```st_distance()``` function to generate a distance matrix. We'll assign the matrix to an object named ```boulder_hospital_distancematrix```:

```{r}
# Generates a distance matrix that contains information on the pairwise Euclidean distances between all of Boulder's hospitals and assigns the matrix to a new object named "boulder_hospital_distancematrix"
boulder_hospital_distancematrix<-st_distance(boulder_hospitals)
```

To make things easier to interpret, we'll label the matrix's rows and columns with the actual names of the corresponding hospitals (rather than index numbers). 

```{r}
# extracts names of Boulder hospitals and uses those names as row labels
rownames(boulder_hospital_distancematrix)<-boulder_hospitals$NAME
# extracts names of Boulder hospitals and uses those names as column labels
...
```{r}
# Uses data in "boulder_hospital_distancematrix" to extract distance between Longmont United Hospital and Boulder Community Hospital
boulder_hospital_distancematrix["Longmont United Hospital", "Boulder Community Hospital"]
```

Note that the unit of analysis is in feet, which corresponds to the unit of analysis used by the coordinate reference system of our data. 

## Extracting distances between features in different datasets

We can also extract information about distances between points in different datasets. To see this, let's bring in another dataset, which contains information on the location of Boulder County fire stations, which is available on the Boulder County GIS Data website [here](https://opendata-bouldercounty.hub.arcgis.com/datasets/bouldercounty::fire-stations/about). Go ahead and download this data into your working directory, read this point layer of fire stations R Studio, and assign it to an object named ```boulder_fire_stations```:

```{r, echo=-1}
setwd("class_notes/class2/data/fire_stations")
# Reads in point layer of Boulder fire stations and assigns it to a new object named "boulder_fire_stations"
boulder_fire_stations<-st_read("Fire_Stations.shp")
```

Let's reproject the fire station data into EPSG:2231 (the projected coordinate system appropriate for Boulder) and assign the reprojected data to an object named ```boulder_fire_stations_projected```:

```{r}
# reprojects "boulder_fire_stations" object into CRS associated with EPSG: 2231, and assigns the reprojected data to a new object named "boulder_fire_stations_projected"
...
```{r}
# plots "boulder_fire_stations_projected" in red, "boulder_hospitals" in blue, against the backdrop of Boulder County, and assigns this plot to a new object named "firestations_hospitals"
...
# prints "firestations_hospitals" plot
firestations_hospitals
```

Now, if we want to extract a distance matrix that contains the pairwise Euclidean distances between Boulder's hospitals and its fire stations, we can pass the names of the relevant objects to ```st_distance```. We'll assign the matrix to an object named ```boulder_hospital_firstation_distancematrix```: 

```{r}
# Generates a distance matrix between Boulder's hospitals and fire stations and assigns it to a new object named "boulder_hospital_firstation_distancematrix"
...
```{r}
# Extracts Boulder hospital names and assigns them as row labels
rownames(boulder_hospital_firstation_distancematrix)<-boulder_hospitals$NAME
# Extracts Boulder fire station names and assigns them as column labels
...
```{r}
# Uses "boulder_hospital_firstation_distancematrix" to find distance between Boulder Medical Center (hospital) and "Nederland Station 2" (fire station)
...
```{r}
# transforms "boulder_hospital_firstation_distancematrix" into a data frame and assigns the result to a new object named "boulder_hospital_firstation_distancematrix_df"
...
```{r}
# creates a vector containing distances to closest fire station for each hospital and assigns the result to a new object named "HOSPITAL_closest_fire_station"
HOSPITAL_closest_fire_station<-apply(boulder_hospital_firstation_distancematrix_df, 1, FUN=min)
```

```{r, echo=T}
# transforms "HOSPITAL_closest_fire_station" vector into a data frame
...
```{r}
# creates a vector containing distances to closest hospital for each fire station and assigns the result to a new object named "FIRESTATION_closest_hospital"
FIRESTATION_closest_hospital<-apply(boulder_hospital_firstation_distancematrix_df, 2, FUN=min)
```

```{r}
# transforms "FIRESTATION_closest_hospital" vector into a data frame
...
```{r}
# extracts area of Boulder county in square feet and assigns the value to new object named "boulder_area"
...
```{r}
# changes units of "boulder_area" object to square kilometers and assigns the result to a new object named "boulder_area_km"
...
```{r}
# prints value of "boulder_area_km"
boulder_area_km
```

Let's say we want the area of Boulder in square miles:

```{r}
# changes units of "boulder_area" to square miles and assigns the result to "boulder_area_sqmiles"
boulder_area_sqmiles<-set_units(boulder_area, "mi^2")
```

```{r}
# prints value of "boulder_area_sqmiles"
boulder_area_sqmiles
```
...
# calculates area of Boulder census tracts and assigns the resulting dataset to a new object named "boulder_tract_areas"
...
```{r}
# Changes unit of analysis of census tract areas in "boulder_tract_areas" to square kilometers
...
# Generating Distance Buffers

A distance buffer delimits the area around a geographic feature that is within a specified distance. To see how we can create and use such buffers, let's first return to our hospitals and fire stations:
...
Let's say we want to create a buffer, with a radius of one mile, for each of the hospitals; in other words, we want to delimit areas that are within one mile of each of our hospitals. To do so, we can pass ```boulder_hospitals``` to the ```st_buffer()``` function, and then specify the desired distance using the "dist" argument (since the dataset's units are in feet, we express the one mile radius in feet). We'll assign these buffers to a new object named ```hospital_buffers```:

```{r}
# Creates one mile distance buffers around Boulder hospitals, and assigns the result to a new object named "hospital_buffers" 
...
```{r}
# Plots Boulder hospitals and one mile distance buffers for Boulder hospitals against the backdrop of Boulder county
tm_shape(boulder_county_projected)+
  tm_polygons()+
tm_shape(boulder_hospitals)+
  tm_dots("blue")+
...
Now, let's create one mile buffers for our fire stations to delimit areas within one mile of a fire station; we'll assign the result to a new object named ```fire_station_buffers```:

```{r}
# generates one mile buffers for fire stations in "boulder_fire_stations_projected" dataset and assigns the result to "fire_station_buffers"
fire_station_buffers<-st_buffer(boulder_fire_stations_projected, dist=5280)
```

Let's now plot our fire station buffers, along with the underlying points:

```{r}
# Generates a plot of fire stations, and their associated one mile buffers, against the backdrop of Boulder county
tm_shape(boulder_county_projected)+
  tm_polygons()+
tm_shape(boulder_fire_stations_projected)+
  tm_dots("red")+
...
```{r}
# Generates a plot of fire stations, hospitals, and their associated buffers against the backdrop of Boulder county
tm_shape(boulder_county_projected)+
  tm_polygons()+
tm_shape(boulder_fire_stations_projected)+
  tm_dots("red")+
...
Now that we have our buffers, we can do some interesting visualization and analysis. For example, let's say we want to visually represent the geographic areas in Boulder county that are within one mile of a hospital AND within one mile of a fire station, and calculate the percentage of Boulder County's land area that is within one mile of both a hospital and a fire station. To do so, let's first compute the intersection between our buffers and assign it to an object named ```hospital_firestation_intersection```:

```{r}
# calculates the intersection of "hospital_buffers" and "fire_station_buffers" and assigns the result to a new object named "hospital_firestation_intersection"
...
```{r}
# Plots "hospital_firestation_intersection" (i.e. the area in Boulder county within one mile of a hospital and fire station) against the backdrop of Boulder County
tm_shape(boulder_county_projected)+
  tm_polygons()+
...
Then, let's dissolve the borders so we have a cleaner image, and assign it to an object named ```hospital_firestation_intersection_dissolve```:

```{r}
# Dissolves borders of "hospital_firestation_intersection" and assigns the object to "hospital_firestation_intersection_dissolve"
...
```{r}
# Creates plot of "hospital_firestation_intersection_dissolve" against backdrop of Boulder County
tm_shape(boulder_county_projected)+
  tm_polygons()+
...
```{r}
# Extracts area of "hospital_firestation_intersection_dissolve" and assigns value to "fire_hospital_intersection_area"
...
Since we want to represent this area in square miles, let's pass ```fire_hospital_intersection_area``` to the ```set_units()``` function:

```{r}
# changes units of "fire_hospital_intersection_area" to square miles and assigns the resulting the value to an object named "fire_hospital_intersection_area_sqmiles"
fire_hospital_intersection_area_sqmiles<-set_units(fire_hospital_intersection_area, "miles^2")

# prints value of "fire_hospital_intersection_area_sqmiles"
fire_hospital_intersection_area_sqmiles
```

We can see that 15.83 sq miles of land area in Boulder County is within 1 mile of both a fire station AND a hospital. Recall that we learned from an earlier calculation that Boulder County is roughly 740 square miles. So, it follows that roughly 2.26% of Boulder County is within both a fire station and a hospital. 

As an exercise, see if you can figure out the percentage of land area within Boulder County that is within 1 mile of a fire station OR a hospital. 
