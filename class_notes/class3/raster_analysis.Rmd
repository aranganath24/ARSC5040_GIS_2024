...
Please load the following packages. If any of these packages are not already installed, please remember to install them first using the ```install.packages()``` function.

```{r, message=F, warning=F, results=F}
# loads lesson libraries
library(sf)
library(tidyverse)
library(raster)
...
* High resolution population rasters of New York City for the years 2009 and 2019
...
* In the final row, we see that the minimum population value associated with a cell/grid is 0 (i.e. unpopulated cells) and the maximum cell/grid population is 2668.463. It may seem strange that there are decimals in a population count (where we would naturally expect whole numbers), but this is an artifact of the model used to generate these cell-level population estimates. 
...
Now that we've created the data frame, we can generate a histogram that visualizes the frequency distribution of cell-population counts with the following code, which uses the ```ggplot2``` package: 
...
In the code above, we use ```ggplot()``` to initiate a new *ggplot* object, and then call the  ```geom_histogram()``` function to create a histogram based on the "nyc_2019" population column within the ```nyc_2019_population_df``` object. If you'd like more or less bins, you can adjust the "bins" argument accordingly. 
...
```{r, echo=-1}
tmap_mode("plot")
# plots "nyc_2019_population" object using 10 equal intervals and a "Yellow-Green-Blue" palette
tm_shape(nyc_2019_population)+
...
```{r}
# modifies the plot using a reversed "Yellow-Green-Blue" palette
tm_shape(nyc_2019_population)+
  tm_raster(style="equal", n=10, palette="-YlGnBu")
```

Let's experiment with some other color palettes. Below, we'll reduce the number of data partitions to 8 (```n=8```), and use an inverted "Purple to Red" palette (```palette="-PuRd"```):

```{r, echo=-1}
tmap_mode("plot")
# makes a plot of "nyc_2019_population" using 8 equal data partitions and a "Purple to Red" palette
tm_shape(nyc_2019_population)+
...
```{r}
# adds additional formatting to "nyc_2019_population" to create map
tm_shape(nyc_2019_population)+
...
Of course, there is a lot more that could be done (for example, we could add a credits section) using *tmap*; once the raster plot showing the spatial distribution of the population is customized to your satsifaction and ready to be exported, assign the map to an object and use the ```tmap_save()``` function that we discussed in Class 1 to write the map to your working directory. 
...
# reads in polygon layer of NYC boroughs and assigns it to an object named "nyc_boroughs"
...
```{r}
# converts "manhattan" sf object to a spatial data frame and assigns the newly created manhattan spatial data frame to an object named "manhattan_spatialdf"
manhattan_spatialdf<-as(manhattan, "Spatial")
```

Now that we have our spatial data frame of Manhattan, we'll use it to clip our NYC raster to the bounding box of ```manhattan_spatialdf```. We'll clip our NYC raster (```nyc_2019_population```) to the bounding box of ```manhattan_spatialdf``` using the ```crop()``` function, and assign the cropped raster to a new object named ```manhattan_cropped```:

```{r}
# reduces "nyc_2019_population" raster to extent of "manhattan_spatialdf" bounding box using "crop" function
manhattan_cropped<-crop(nyc_2019_population, manhattan_spatialdf)
```
...
manhattan_final_raster<-mask(manhattan_cropped, manhattan_spatialdf)
```
...
```{r}
# Use raster math to calculate cell-level population change between 2019 and 2009, and use this calculation to create a new population change raster that's assigned to an object named "population_change_2009_2019"
...
```{r}
# creates a raster of cell-level percentage change in the population from 2009 to 2019, and assigns it to a new object named "population_pct_change_2019_2009" 
...
```{r}
# prints metadata for "population_pct_change_2019_2009" raster object
...
```{r}
# Plots "population_pct_change_2019_2009"
...
We now have a gridded map that highlights cells experiencing greater than 100% population growth from 2009 to 2019 in green. There are certain parts of this map we may want to inspect more closely (for instance, there’s something strange going on in a part of Central Park), but as a first cut, this
raster map gives us a starting point for undertaking a granular study of population
changes in New York City. One might imagine that such analysis could be especially
...
It is advisable to benchmark the results of Zonal Statistics calculations against information from
...
to missed steps, a misaligned projection, an incorrect input raster etc). Here, the estimated population of NYC in 2019 that is derived using our raster dataset and zonal statistics (8,309,940) is very close to an estimate of the City's 2019 population produced by the [US Census](https://www.nyc.gov/assets/planning/download/pdf/planning-level/nyc-population/new-population/current-populatiion-estimattes.pdf) (8,336,817). We would expect some discrepancy between the Census estimate and our raster-based estimate (due, for example, to different models and assumptions, or differences in the time of year in which the measurement was taken). However, the fact that the difference is very small (i.e. less than 0.5%) should give us some confidence that our zonal statistics calculations are working correctly. 
...
The real power of zonal statistics is in its ability to calculate granular spatial information for areas for which we do not have "off-the-shelf" official data. Let's consider an example of how we can use zonal statistics to answer a question that would have been more challenging to answer using other data sources or methods.

In particular, let's say that you're a city planner, and want to find out the percentage of New York City's population that lives more than 500 meters from a subway station. A question like this implicates
important issues relating to New York City’s quality of life, the accessibility of public
infrastructure, and fairness and equity. It would be difficult to answer without GIS data and methods, but can be addressed in a relatively straightforward way with methods that bring together raster data and vector data using zonal statistics.
...
# read in NYC subway stops and assign to a new object named "nyc_subway_stops"
...
```{r}
# Create 500 m buffers around subway stations and assigns buffers to a new object named "subway_500_buffers"
...
```{r}
# Plot dissolved buffers in grey along with subway stops, with backdrop of NYC boroughs
tm_shape(nyc_boroughs)+
  tm_polygons()+
...
```{r}
# Plots dissolved buffer layer in blue
tm_shape(nyc_boroughs)+
  tm_polygons()+
tm_shape(subway_500_buffers_dissolved)+
  tm_borders("blue")
...
```{r}
# Transform "subway_500_buffers_dissolved" to same CRS as "nyc_2019_population" raster amn assign reprojected buffers to new object named "subway_500_buffers_dissolved_4326"  (EPSG: 4326)
...
```{r}
# prints value of "nyc_pop_within_buffer", which contains the number of residents within 500 meters of a subway stop
nyc_pop_within_buffer
```

So, 4,564,042 lived within 500 metres of a subway stop in NYC in 2019. Recall that the total population of NYC, based on our raster calculations, is given by ```sum(nyc_boroughs_population_zonal)```, which we calculated earlier; we'll assign this value to ```total_population_2019```:

```{r, message=F, results=F, warning=F}
# calculates total population of NYC using "nyc_boroughs_population_zonal" and assigns the value to new object named "total_population_2019"
total_population_2019<-sum(nyc_boroughs_population_zonal)
```

To calculate the percentage of the City's population *within* 500m of a subway stop, we can simply divide ```nyc_pop_within_buffer``` by ```total_population_2019``` and multiply by 100. We'll assign this percentage to an object named ```percentage_within_buffer```:
...
**Acknowledgement**: A previous version of this lesson was first concieved and developed at a workshop that I taught through New York University Libraries' [Department of Data Services](https://library.nyu.edu/departments/data-services/) during a Postdoctoral Fellowship sponsored by the [Council on Library and Information Resources](https://www.clir.org) and [New York University Libraries](https://library.nyu.edu).  
